...
@section('main-content')
	<div class="container-fluid spark-screen">
		<div class="row">
          ...
          @endif
          <form name="form_edit_user" method="POST" action="{{ route('users.update', $user) }}" accept-charset="UTF-8"  user="form">
            {{ csrf_field() }}
					  <div class="box-body">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-sm-8">
                          @section('someSection')
                            {{ $varold = old('tenant_id',$user->tenant_id) }}
                          @endsection
                          <select class="form-control" id="tenant_id" name="tenant_id">
                              <option value="0" <?php if ($varold=='0') echo 'selected';?>>- Seleccione -</option>
                              @foreach ($tenants as $tenant)
                                <option value="{{ $tenant->id }}" <?php if ($varold == $tenant->id) echo 'selected';?>>{{$tenant->id."-".$tenant->name}}</option>
                              @endforeach    
                          </select>
                          <p class="errorName text-center alert alert-danger hidden"></p>
                      </div>
